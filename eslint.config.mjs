...
const eslintConfig = [
  {
    ignores: [
      ...
      '**/backups/**',
      'next-env.d.ts',
      // Generated by `pnpm run generate:types`; it ships its own blanket
      // `/* eslint-disable */`, so linting it reported nothing anyway. Ignoring
      // it outright keeps that suppression from reading as hand-written code
      // that violates the no-inline-disable rule in src/AGENTS.md. Already
      // excluded from Prettier and from the license-header spec for the same
      // regeneration reason.
      'src/payload-types.ts',
      // Generated by `pnpm run generate:importmap` and by Payload's admin
      // bundler. `import-x/no-duplicates` merges its two dozen duplicate
      // imports on every `--fix`, so `pnpm run lint` left the tree dirty after
      // every run — including from the pre-push hook, once the push had
      // already succeeded. Excluded from Prettier and the license-header spec
      // for the same regeneration reason.
